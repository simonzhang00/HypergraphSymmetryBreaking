...
from hsb.utils import utils_simplicial, utils_hypergraph, networkx_cycles
...
def construct_hypertransition_matrix(edge_list, num_vertices, device, alpha= 0.0000001):
    ...
    D_v_inv = torch.sparse_coo_tensor(torch.tensor([list(range((D_v_inv).size(0))), list(range((D_v_inv).size(0)))]).to(device), D_v_inv).to(device)
...
def load_data(dataset= 'contact-high-school', pos_embedding= "laplacian_emap", max_path_len= 20, max_cycle_len= 20, tuple_size=3, S=10, K=100, cycle_sampling= "hyperpagerank"):
    ...
    edge_list = [(int(u), int(v)) for s in open_train_ for u in s for v in s if int(u) < int(v)]
