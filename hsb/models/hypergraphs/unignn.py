...
import hsb
from hsb.nn import UniGCNConv, UniGATConv, UniSAGEConv, UniGINConv, MultiHeadWrapper
...
class UniGCN(nn.Module):
    ...
    def __init__(
        ...
        self.layers.append(UniGCNConv(hid_channels, hid_channels, use_bn=use_bn, drop_rate=drop_rate))
        self.layers.append(UniGCNConv(hid_channels, hid_channels, use_bn=use_bn, drop_rate=drop_rate))
        self.layers.append(UniGCNConv(hid_channels, hid_channels, use_bn=use_bn, drop_rate=drop_rate, is_last= True))
        # self.layers.append(UniGCNConv(hid_channels, num_classes, use_bn=use_bn, is_last=True))

    def forward(self, X: torch.Tensor, hg: "esp.Hypergraph") -> torch.Tensor:
...
class UniSAGE(nn.Module):
    ...
    def __init__(
        ...
        self.layers.append(UniSAGEConv(hid_channels, hid_channels, use_bn=use_bn, drop_rate=drop_rate))
        self.layers.append(UniSAGEConv(hid_channels, hid_channels, use_bn=use_bn, drop_rate=drop_rate))
        self.layers.append(UniSAGEConv(hid_channels, hid_channels, use_bn=use_bn, drop_rate=drop_rate))
        self.layers.append(UniSAGEConv(hid_channels, hid_channels, use_bn=use_bn, drop_rate=drop_rate))
