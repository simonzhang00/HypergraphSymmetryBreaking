...
def draw_graph(
    ...
    """
    assert isinstance(g, esp.Graph), "The input object must be a DHG's graph object."
...
def draw_digraph(
    ...
    Args:
        ...
        ``pull_center_strength`` (``float``): The center pull strength. Defaults to ``1.0``.
    """
    assert isinstance(g, esp.DiGraph), "The input object must be a DHG's digraph object."
...
def draw_bigraph(
    ...
    """
    assert isinstance(g, esp.BiGraph), "The input object must be a DHG's bigraph object."
    assert e_style in ["line"], "e_style must be 'line'"
    assert g.num_e > 0, "g must be a non-empty structure"
    fig, ax = plt.subplots(figsize=(6, 6))
...
def draw_hypergraph(
    ...
    """
    assert isinstance(hg, esp.Hypergraph), "The input object must be a DHG's hypergraph object."
