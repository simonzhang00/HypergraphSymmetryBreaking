...
import hsb
from hsb.metrics import BaseEvaluator
from hsb.utils import default_log_formatter
from hsb.structure.base import load_structure
...
class BaseTask:
    ...
    def run(self, max_epoch: int, num_trials: int = 1, direction: str = "maximize"):
        ...
        """
        self.logger.info(f"Random seed is {esp.random.seed()}")
        sampler = TPESampler(seed=esp.random.seed())
