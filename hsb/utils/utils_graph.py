...
def make_train_test_data(name= 'FB15k-237'):
    if name=='FB15k-237':
        ...
        data = dataset[0]
        train_edge_index= data.edge_index[:,:int((0.80)*data.edge_index.size(1))] #data.edge_index[train_idx.expand(2,-1)] #(torch.tensor([int(r in train_idx) for r in data.edge_index[0]])*torch.tensor([int(c in train_idx) for c in data.edge_index[1]])).expand(2,data.edge_index.size(1)) * data.edge_index
        valid_edge_index= data.edge_index[:,int((0.80)*data.edge_index.size(1)):int((0.85)*data.edge_index.size(1))]#data.edge_index[valid_idx.expand(2,-1)]  #(torch.tensor([int(r in valid_idx) for r in data.edge_index[0]])*torch.tensor([int(c in valid_idx) for c in data.edge_index[1]])).expand(2,data.edge_index.size(1)) * data.edge_index
        test_edge_index= data.edge_index[:,int((0.85)*data.edge_index.size(1)):] #data.edge_index[test_idx.expand(2,-1)] #(torch.tensor([int(r in test_idx) for r in data.edge_index[0]])*torch.tensor([int(c in test_idx) for c in data.edge_index[1]])).expand(2,data.edge_index.size(1)) * data.edge_index
