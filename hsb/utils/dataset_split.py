...
from hsb.structure.graphs import BiGraph
...
def split_by_num(
    ...
    Examples:
        >>> import numpy as np
        >>> from hsb.utils import split_by_num
        >>> num_v = 100
...
def split_by_ratio(
    ...
    Examples:
        >>> import numpy as np
        >>> from hsb.utils import split_by_ratio
        >>> num_v = 100
        >>> v_label = np.random.randint(0, 3, num_v) # 3 categories
...
def split_by_num_for_UI_bigraph(g: BiGraph, train_num: int):
    ...
    Examples:
        >>> import hsb
        >>> from hsb.utils import edge_list_to_adj_list, split_by_num_for_UI_bigraph
        >>> g = esp.random.bigraph_Gnm(5, 8, 20)
...
def split_by_ratio_for_UI_bigraph(g: BiGraph, train_ratio: float):
    ...
    Examples:
        >>> import hsb
        >>> from hsb.utils import edge_list_to_adj_list, split_by_ratio_for_UI_bigraph
        >>> g = esp.random.bigraph_Gnm(5, 8, 20)
        >>> edge_list_to_adj_list(g.e[0])
